...
- **System-defined roles (tree hierarchy, NOT a linear chain):**
  - ORGADMIN → ACCOUNTADMIN (two branches below):
    - Branch 1: ACCOUNTADMIN → SECURITYADMIN → USERADMIN → PUBLIC
    - Branch 2: ACCOUNTADMIN → SYSADMIN → PUBLIC
  - Note: SYSADMIN does NOT inherit from USERADMIN — they are separate branches
