...
### What can be shared (objects you can GRANT to a SHARE):
...
### What CANNOT be shared (NEVER goes inside a Share):
- **Warehouses** -- compute NEVER travels in a share
- **Tasks** -- automation stays in your account
- **Streams** -- change tracking stays in your account
- **Pipes (Snowpipe)** -- ingestion stays in your account
- **Stages** -- storage references stay in your account
- Regular (non-secure) views (by default)

### ELI5 -- Data vs Compute: What Goes in the Share?

Think of a Share as a lunchbox. You can put FOOD in it (data objects), but you cannot put the MICROWAVE in it (compute).

**Food (goes in the Share):** Tables, Secure Views, Secure UDFs, Dynamic Tables, Iceberg Tables -- these are all DATA objects. The consumer opens the lunchbox and reads them.

**Microwave (stays in YOUR kitchen):** Warehouses, Tasks, Streams, Pipes -- these are COMPUTE and AUTOMATION objects. They do work, they cost credits, and they never leave your account. The consumer must bring their own microwave (their own warehouse) to heat the food (query the data).

Don't confuse "sharing data" with "sharing compute." Even when 50 consumers access the same share, each one uses their own warehouse. The provider's warehouse is never touched by consumer queries.

Precedence Rule: The Share object is the same whether the consumer is a full Snowflake account OR a Reader Account. The only difference is who PAYS for compute -- not what goes in the Share.
...
**Exam trap**: "Can you share a Stream or a Task?" → NO. Streams and Tasks are automation/tracking objects that stay in the provider's account. IF YOU SEE "stream" or "task" + "added to share" → WRONG. Also avoid creating secure views on streams and sharing those views -- Snowflake explicitly warns against this.
...
- Reader account can ONLY consume data from the provider that created it
- No hard limit on number of Reader accounts per Provider

### ELI5 -- Where Does the Warehouse Come From in a Reader Account?

The Share (lunchbox) is EXACTLY the same whether the consumer has Snowflake or not. Tables, Secure Views, Secure UDFs -- same objects, same rules. You NEVER put a warehouse inside the share.

**Normal sharing (consumer HAS Snowflake):** You send the lunchbox. The consumer heats it in THEIR microwave (their warehouse). They pay their own electric bill.

**Reader Account (consumer does NOT have Snowflake):** You send the SAME lunchbox (the share doesn't change). But since the consumer has no kitchen, you set up a mini-kitchen for them (CREATE WAREHOUSE inside the Reader Account). The electric bill for that mini-kitchen goes to YOU (the provider).

The key insight: the warehouse in the Reader Account was NOT "shared" from the provider's account. It was created from scratch inside the Reader Account, with billing going to the provider. The share itself still contains zero compute.

Don't confuse "provider pays for Reader Account compute" with "provider's warehouse is in the share." The share never contains a warehouse. The provider creates a NEW warehouse inside the Reader Account and pays for it.
...
**Exam trap**: "What objects can be added to a Share for a Reader Account?" → The SAME objects as any share (Tables, Secure Views, Secure UDFs, etc.). IF YOU SEE "warehouse" + "added to share" or "share includes compute" → WRONG because a share NEVER contains compute objects, regardless of consumer type.
**Exam trap**: "Reader Account uses provider's warehouse?" → NO. A NEW warehouse is created INSIDE the Reader Account. IF YOU SEE "provider's warehouse" + "reader account queries" → WRONG because the Reader Account has its own warehouse (billed to provider, but separate).
...
### Parameter: MIN_DATA_RETENTION_TIME_IN_DAYS
- Set at **account level ONLY** (ACCOUNTADMIN required)
- Acts as a FLOOR -- objects cannot go below this value
- Formula: effective retention = MAX(DATA_RETENTION_TIME_IN_DAYS, MIN_DATA_RETENTION_TIME_IN_DAYS)
- Enterprise+ only

### ELI5 -- The City Law vs. The Room Thermostat (MIN_DATA_RETENTION)

Imagine you own a building in a cold city.

**The thermostat (DATA_RETENTION_TIME_IN_DAYS):** You can put a thermostat at every level -- the Building (Account), the Floor (Database), the Corridor (Schema), or inside the Room (Table). Each place gets its own temperature.

**The city law (MIN_DATA_RETENTION_TIME_IN_DAYS):** The city passes a law: "No place in this city can go below 7 degrees." This law lives ONLY at the city level (Account). The city does not go room by room to enforce it -- it's one global rule at the top.

Why does Snowflake do this? Safety net. If a junior engineer enters the Sales Schema and sets DATA_RETENTION to 0 (turning off Time Travel and deleting all history), the system checks the "city law" at account level and says: "The minimum is 7 days. I'm ignoring the zero and keeping 7 days."

Don't confuse MIN_DATA_RETENTION with DATA_RETENTION. DATA_RETENTION can be set at any level (account, database, schema, table). MIN_DATA_RETENTION can ONLY be set at account level. MIN does not replace DATA_RETENTION -- it just ensures nobody goes below the minimum. If DATA_RETENTION is 30 and MIN is 7, the effective retention is 30 (not 7).
...
**Exam trap**: "MIN_DATA_RETENTION set at schema level?" → WRONG. It can ONLY be set at account level. IF YOU SEE "MIN_DATA_RETENTION" + "schema" or "database" or "table" → WRONG because MIN_DATA_RETENTION is an account-level-only parameter.
**Exam trap**: "DATA_RETENTION is 0 but MIN_DATA_RETENTION is 7. Effective retention?" → 7 days. The formula is MAX(0, 7) = 7. IF YOU SEE "0 days" when MIN_DATA_RETENTION is set higher → WRONG because the MIN acts as a floor.
