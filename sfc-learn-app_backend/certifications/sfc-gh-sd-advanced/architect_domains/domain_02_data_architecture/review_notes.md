...
- **Managed (Snowflake-managed):** Snowflake manages the Iceberg metadata + data files
  - Full DML support (INSERT, UPDATE, DELETE, MERGE)
  - Snowflake handles compaction, snapshot management
  - Stored in Snowflake-managed storage or customer external volume
...
- If you drop and recreate same-named object, the old one is still undrop-able (uses internal versioning)
