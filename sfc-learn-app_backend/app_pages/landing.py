...
f'<div style="display:inline-block; margin-bottom:6px;">{SF_LOGO}</div>'
f'<h1 style="font-size:2rem; margin:0; color:#E0F7FA;">{t("app_title")}</h1>'
f'<p style="color:#80DEEA; font-size:0.95rem; margin:4px 0 0;">{t("app_subtitle")}</p>'
...
f'<h2 style="color:#E0F7FA; margin:8px 0 6px; font-size:1.25rem;">{t("certifications")}</h2>'
f'<p style="color:#80DEEA; font-size:0.82rem; margin:0 0 10px;">{t("certifications_desc")}</p>'
f'<div style="display:flex; gap:5px; justify-content:center; flex-wrap:wrap;">{_cert_badges}</div>'
'</div>', unsafe_allow_html=True)
    if st.button(f"{t('enter_certifications')}", key="go_cert", use_container_width=True, type="primary"):
        ...
        # Ensure _active_cert is set so dashboard loads correct cert
        if "_active_cert" not in st.session_state:
            st.session_state._active_cert = "core"
...
' display:flex; flex-direction:column; justify-content:center;">'
f'<h2 style="color:#E8F5E9; margin:8px 0 6px; font-size:1.25rem;">{t("project_preparation")}</h2>'
f'<p style="color:#A5D6A7; font-size:0.82rem; margin:0 0 10px;">{t("project_prep_desc")}</p>'
...
' display:flex; flex-direction:column; justify-content:center;">'
f'<h2 style="color:#F3E5F5; margin:8px 0 6px; font-size:1.25rem;">{t("learn")}</h2>'
f'<p style="color:#CE93D8; font-size:0.82rem; margin:0 0 10px;">{t("learn_desc")}</p>'
'<div style="display:flex; gap:5px; justify-content:center; flex-wrap:wrap;">'
...
'<h3 style="color:#E0F7FA; font-size:1.1rem; margin:0;">Certifications</h3>'
'</div>', unsafe_allow_html=True)
...
for cat_key in ["core", "advanced", "specialist"]:
    ...
    rgba = ','.join(str(int(meta['color'].lstrip('#')[i:i+2], 16)) for i in (0, 2, 4))

    st.markdown(
f'<div style="margin:14px 0 8px; padding:10px 16px; border-left:4px solid {meta["border"]};'
f' background:rgba({rgba},0.08); border-radius:0 8px 8px 0;">'
f'<strong style="color:{meta["color"]}; font-size:1.1rem;">{meta["title"]}</strong>'
f'<span style="color:#9CA3AF; font-size:0.82rem; margin-left:10px;">{cat_desc}</span>'
'</div>', unsafe_allow_html=True)

    cols = st.columns(max(1, min(len(certs), 4)))
    for i, cert in enumerate(certs):
        ...
        with cols[i % len(cols)]:
            st.markdown(
f'<div style="background:#11263B; border-radius:10px; padding:14px; text-align:center;'
f' border-top:3px solid {cert.get("color","#6B7280")}; opacity:{opacity}; min-height:140px;">'
f'<p style="color:#E0F7FA; font-weight:600; font-size:0.82rem; margin:10px 0 2px;">{cert.get("name","")}{tag}</p>'
f'<p style="color:#546E7A; font-size:0.68rem; margin:0 0 3px;">{cert.get("code","")}</p>'
f'<p style="color:#80DEEA; font-size:0.72rem; margin-top:5px;">{info}</p>'
'</div>', unsafe_allow_html=True)
            if is_available:
                if st.button(f"Enter {cert.get('name','')}", key=f"cert_enter_{cat_key}_{i}", use_container_width=True):
                    # Use _pending_cert (NOT cert_select) to avoid
                    # StreamlitAPIException -- cert_select is a widget key
                    # owned by the sidebar selectbox in streamlit_app.py.
                    st.session_state._pending_cert = cert.get("full_name", "")
                    st.session_state.app_mode = "certifications"
                    st.switch_page("app_pages/dashboard.py")

# ── Sidebar: Snowflake logo + connection ──
st.sidebar.markdown(
f'<div style="text-align:center; padding:8px 0;">{SF_LOGO}</div>',
